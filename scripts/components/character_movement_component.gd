...
func tick_dash_cooldown(_delta: float) -> void:
	if actor._dash_cooldown_timer > 0.0:
		actor._dash_cooldown_timer = maxf(actor._dash_cooldown_timer - _delta, 0.0)

func try_start_dash() -> bool:
	if actor._is_dashing:
		return false
...
func start_dash() -> void:
	actor._is_dashing = true
	actor._dash_timer = actor._dash_duration
	actor._dash_cooldown_timer = actor._dash_cooldown
	...
	actor.velocity.x = dir * actor._dash_speed
	actor.velocity.y = 0
	...
	actor._dash_ghost_timer = 0.05

func process_dash(_delta: float) -> void:
	actor._dash_timer -= _delta
	actor.velocity.y = 0

	actor._dash_ghost_timer -= _delta
	if actor._dash_ghost_timer <= 0.0:
		actor._dash_ghost_timer = 0.05
		global.spawn_ghost(actor._character_texture)

	if actor._dash_timer <= 0.0:
		actor._is_dashing = false
		actor.velocity.x = 0

func vertical_movement(_delta: float, allow_jump: bool = true) -> void:
	if actor.is_on_floor():
		if not actor._on_floor:
			...
			actor._on_floor = true

		actor._jump_count = 0

	if not actor.is_on_floor():
		actor._on_floor = false
		actor.velocity += actor.get_gravity() * _delta

	if allow_jump and actor.is_in_state("normal") and Input.is_action_just_pressed("jump") and (actor._jump_count < 2):
...
func horizontal_movement(allow_input: bool = true) -> void:
	...
	if not allow_input:
		actor.velocity.x = move_toward(actor.velocity.x, 0, actor._speed)
		return

	var _direction := Input.get_axis("move_left", "move_right")
	if _direction:
		actor.velocity.x = _direction * actor._speed
		return
